 ...
 * XTAL 27.000 Mhz
 *****************************************************************************/
 const bool Si5351mcu::init(void) {
    // init with the default freq
    return init(int_xtal);
}
 ...
 *****************************************************************************/
 const bool Si5351mcu::init(uint32_t nxtal) {
...
void Si5351mcu::setFreq(uint8_t clk, uint32_t freq) {
    ...
    ****************************************************************************/
    ...
    if (omsynth[clk] != outdivider || o_Rdiv[clk] != R ) {
        ...
        // HEX makes it easier to human read on bit shifts
        uint8_t reg_bank_42[] = { 
          (uint8_t)0,                         // Bits [15:8] of MS0_P3 (always 0) in register 42
          (uint8_t)1,                         // Bits [7:0]  of MS0_P3 (always 1) in register 43
          (uint8_t)(((MSx_P1 & 0x030000L ) >> 16) | R),  // Bits [17:16] of MSx_P1 in bits [1:0] and R in [7:4] | [3:2]
          (uint8_t)((MSx_P1 & 0xFF00) >> 8),    // Bits [15:8]  of MSx_P1 in register 45
          (uint8_t)(MSx_P1 & 0xFF),             // Bits [7:0]  of MSx_P1 in register 46
          (uint8_t)0,                         // Bits [19:16] of MS0_P2 and MS0_P3 are always 0
          (uint8_t)0,                         // Bits [15:8]  of MS0_P2 are always 0
          (uint8_t)0                          // Bits [7:0]   of MS0_P2 are always 0
        };
